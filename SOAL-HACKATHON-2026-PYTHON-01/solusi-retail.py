...
daily_sales = (
    df.groupby(['nama_produk', 'tgl_transaksi'], sort=False)['total_nilai']
    .sum()
    .reset_index()
)
daily_sales = daily_sales.sort_values(['nama_produk', 'tgl_transaksi']).reset_index(drop=True)

# 2b. Moving Average 3 hari (per produk, min_periods=1 agar MA tersedia sejak hari pertama)
daily_sales['MA'] = (
    daily_sales.groupby('nama_produk')['total_nilai']
    .transform(lambda x: x.rolling(window=3, min_periods=1).mean())
)

# 2c. Identifikasi streak kenaikan MA & hitung max streak per produk (vectorized)
daily_sales['is_up'] = daily_sales.groupby('nama_produk')['MA'].diff() > 0
...
# 2d. Growth Calculation (First-Last keseluruhan data, bukan streak-based)
growth_data = daily_sales.groupby('nama_produk')['MA'].agg(['first', 'last'])
growth_data['growth_percentage'] = ((growth_data['last'] / growth_data['first']) - 1) * 100

# 2e. Gabungkan dan ambil TEPAT 18 BARIS terbaik
df_rising_star = (
    pd.merge(max_streaks, growth_data[['growth_percentage']], on='nama_produk')
    .sort_values(by=['max_streak_days', 'growth_percentage'], ascending=False)
    .head(18)
)

print(f"  Rising Star ditemukan: {len(df_rising_star)} produk")
for _, row in df_rising_star.iterrows():
    print(f"    - {row['nama_produk']}: Streak {row['max_streak_days']}, Growth {row['growth_percentage']:.2f}%")
...
daily_sales['Normalized'] = daily_sales.groupby('nama_produk')['MA'].transform(
...
top3_names = df.groupby('nama_produk')['total_nilai'].sum().nlargest(3).index.tolist()
...
if len(plot_df) > 0:
    ...
    for i, row in enumerate(sorted_report.itertuples()):
        ...
        rank_mapping[nama] = i + 1

    # D. PLOT TOP 3 SALES (ABU-ABU)
    grey_colors = ['#B0B0B0', '#909090', '#707070']

    for idx, (nama_produk, group) in enumerate(
        top3_plot_df.groupby('nama_produk')
    ):
    ...
    # E. PLOT RISING STAR
    for nama_produk, group in plot_df.groupby('nama_produk'):
        line_color = color_mapping.get(nama_produk, default_color)
        rank = rank_mapping.get(nama_produk, '?')
